...
| Tables | 20–24 px rows, muted headers, full-row selection, clear focus | TableBuilder used widely | Focus/copy behavior differed by page | Reusable row/cell focus cursor, arrow navigation, cell-first copy on priority tables | Persistent columns and remaining secondary tables |
...
| Split panes | Resizable rail/inspector with usable center | Resizable side panels existed | Inspector width was not persisted or resettable | Responsive width clamp, persisted width/visibility, Window menu Show/Hide/Reset | Bottom inspector at narrow breakpoint |
| Charts workspace | Controls, plot, readouts, data table, explicit focus | Functionally advanced | Header/focus/empty hierarchy inconsistent | Shared header, full-row table selection, focused-cell highlight, empty range state | Brush/range gesture and richer split resizing |
...
| Fund Detail page | Subject-led header, tabs, facts, listings | Functional multi-section page | Generic title did not identify subject | Subject/source/status header, explicit missing/empty states, selected listing row | Stronger tab styling and two-column overview |
| Jobs/Alerts/Documents | Operational headers and table-first workflows | Functional pages | Inconsistent page identity and empty handling | Shared headers, counts/state badges, full-row selection, empty states | Linked job detail split and document preview split |
...
| Charts | Existing explicit chart row/cell focus | Plot and table focus remain distinct from active subject | No split-layout persistence | Preserved existing chart focus while sharing copy semantics | Brush/range gesture |
...
| Documents | Responsive list and in-app viewer | List selection, focused metadata, double-click viewer | Row-only focus | Stable five-column focus across compact/wide layouts; Enter/double-click opens viewer | Resizable preview split |
...
- The prototype's document preview split, rich chart brush, and icon rail remain deferred.
...
| Charts / series data | Partial | Yes | Yes | No | Existing fixed columns | Yes | Layout state remains chart-specific | Existing behavior preserved | Persistent chart columns |
...
| Fund Detail / distributions | No | Yes | Yes | No | No | Yes | Existing focus only | Behavior retained | Persistent descriptors |
| Fund Detail / documents | No | Yes | Yes | No | No | Yes | Existing focus only | Behavior retained | Persistent descriptors |
...
| Jobs / scheduled and runs | No | Yes | Yes | No | No | Yes | Action columns do not yet share descriptor indexing | Existing focus retained | Persistent descriptors and linked split |
...
| Documents / snapshots | Yes | Yes | Yes | Yes | Visibility; fixed descriptor order | Yes | Compact/wide column counts were unstable | Stable columns; hash/checked hidden by default | In-page preview split |
