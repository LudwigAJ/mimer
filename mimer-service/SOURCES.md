...
> **This file is research, not implemented status.** It catalogs *candidate* endpoints; it does **not** mean an adapter exists or has been verified. For the **implemented** per-source operational truth see `app/sources/source_readiness.py` (`GET /api/v1/data-sources/readiness`); for the **per-fund** live status of VUSA / ISF / JEPG (facts / listing price / NAV / holdings / distributions / documents) see `app/sources/fund_source_coverage.py` (`GET /api/v1/data-sources/fund-coverage`). Notably: NAV is described below as *available on issuer pages* but is **not implemented** in the backend — the coverage matrix marks NAV `planned` and never conflates it with the listing close. IBKR is **deferred** (`planned`, not implemented). A research claim that an endpoint "shows NAV" is never the same as a verified adapter; promotion requires a real `--verify-source` / `verify_fund_sources` fetch+parse.
